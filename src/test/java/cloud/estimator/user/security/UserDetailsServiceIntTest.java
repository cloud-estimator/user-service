...
public class UserDetailsServiceIntTest {
	...
	@Before
	public void init() {
		userOne = User.builder().login(USER_ONE_LOGIN).password(RandomStringUtils.random(8)).activated(true)
				.email(USER_ONE_EMAIL).name("userOne doe").langKey("en").createdBy("junit").build();

		userRepository.save(userOne);

		userTwo = User.builder().login(USER_TWO_LOGIN).password(RandomStringUtils.random(8)).activated(true)
				.email(USER_TWO_EMAIL).name("userTwo doe").langKey("en").createdBy("junit").build();

		userRepository.save(userTwo);

		userThree = User.builder().login(USER_THREE_LOGIN).password(RandomStringUtils.random(8)).activated(false)
				.email(USER_THREE_EMAIL).name("userThree doe").langKey("en").createdBy("junit").build();
	...
	public void assertThatEmailIsPrioritizedOverLogin() {
		UserDetails userDetails = userDetailsService.loadUserByUsername(USER_ONE_EMAIL.toUpperCase(Locale.ENGLISH));
		assertThat(userDetails).isNotNull();
		assertThat(userDetails.getUsername()).isEqualTo(USER_ONE_LOGIN);
	}
